...
export default function MarkingCard({
  ...
  const reqFields = svmode
    ? ["synthesizer", "copyright"]
    : ["name", "vocal", "author", "synthesizer", "copyright", "type"];
  ...
  const loadVideoPreview = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `https://api.xingzhige.com/API/b_parse/?url=${targetUrl}`,
      );
  ...
  return (
    <Card
      id={`record-${index}`}
      className={cn(
        "w-full transition-all duration-300 hover:shadow-lg scroll-mt-24 relative",
        record.status === "auto"
      ...
      <CardContent className="p-4">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="w-full md:w-1/3 flex flex-col gap-2">
            <div className="relative group">
              <a
                href={
                  svmode
                    ? `https://www.bilibili.com/video/av${record.aid}`
          ...
          <div className="w-full md:w-2/3 flex flex-col gap-3">
            <div
              className="font-bold text-lg leading-tight line-clamp-1 pr-16"
              title={record.title}
            >
              {record.title}
            </div>
                ...
                <div key={field.prop} className="flex flex-col space-y-1">
                  ...
                  {(field.type === "tags" || field.type === "tags-hint") && (
                    <MarkingTags
                      ...
                      onChange={(val) => handleChange(field.prop, val)}
                      onInputChange={(val) =>
                        handleInputChange(field.prop, val)
                      }
                      type={field.search || field.prop}
                      useHint={field.type === "tags-hint"}
                      hasError={
                        showFieldError(field.prop) ||
                        !!record[`_unconfirmed_${field.prop}`]
                      }
                    />
                  )}
